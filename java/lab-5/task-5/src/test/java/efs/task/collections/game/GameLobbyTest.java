...
class GameLobbyTest {
    ...
    void shouldSetupBaseLobbyNoDLCHeroesAndTowns() {
        ...
        for (Map.Entry<Town, List<Hero>> entry : gameLobby.getPlayableTownsWithHeroesList().entrySet()) {
            softly.assertThat(entry.getKey().townName()).isEqualTo(alphabeticalTowns.get(iterator++));
            softly.assertThat(entry.getValue().size()).isEqualTo(HEROES_NUMBER_BY_TOWN.get(entry.getKey().townName()));
        }
        softly.assertAll();
    }
